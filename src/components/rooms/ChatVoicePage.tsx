...
export function ChatVoicePage({ inviteLink, view, initialUsername = null, initialProfile = null }: ChatVoicePageProps) {
  ...
  const { users: roomUsers, loading: loadingUsers, error: usersError, refreshUsers } = useRoomUsers(roomId);
  ...
  useEffect(() => {
    ...
    channel
      .on("broadcast", { event: "PASSWORD_CHANGED" }, () => {
  ...
  useEffect(() => {
    if (roomId && roomUsers.length > 0) {
      // console.log(
      //   `[ChatVoicePage] Room users:`,
      //   roomUsers.map((u) => ({ id: u.id, username: u.username, isOnline: u.isOnline }))
      // );
  ...
  useEffect(() => {
    if (
      !roomId ||
      !currentUserData?.userId ||
      currentUserData.userId === "unknown" ||
      currentUserData.userId === "loading"
    ) {
      return;
    }
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if ((response.status >= 500 || response.status === 429) && retryCount < maxRetries) {
              const errorData = await response.text();
              console.warn(
                `[Presence] Failed to update presence (${response.status}), retrying in ${retryDelay}ms... (attempt ${retryCount + 1}/${maxRetries}):`,
                errorData
              );
              setTimeout(() => {
          ...
          if (retryCount < maxRetries) {
            console.warn(
              `[Presence] Error updating presence, retrying in ${retryDelay}ms... (attempt ${retryCount + 1}/${maxRetries}):`,
              error
            );
            setTimeout(() => {
              updatePresence(retryCount + 1);
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-lg font-semibold flex items-center retro-heading">
                  <span className="md:hidden">{roomName || `Pokój ${roomId?.slice(-6).toUpperCase()}`}</span>
                  <TypingAnimation
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                {soundSettings.enabled ? (
                  ...
                  <VolumeOff className="h-3 w-3 sm:mr-1" />
                )}
              ...
              {currentUserRole === "owner" && roomId && (
                <RoomSettingsDialog roomId={roomId} onClearChat={clearChat} requiresPassword={requiresPassword} />
              )}
                  ...
                  >
                    {isVoiceConnected ? (
                      <div className="relative">
                        <Mic
                          className={`w-16 h-16 ${isMuted ? "text-destructive" : "text-[var(--retro-orange-bright)]"}`}
                        />
                        {isMuted && (
                    ...
                    {isVoiceConnected ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-center space-x-4 text-sm">
                          <span
                            className={`flex items-center space-x-1 ${isMuted ? "text-destructive" : "text-[var(--retro-orange-bright)]"}`}
                          >
                            {isMuted ? <MicOff className="h-3 w-3" /> : <Mic className="h-3 w-3" />}
                            <span>{isMuted ? "Wyciszony" : "Mikrofon aktywny"}</span>
                          </span>
                          <span
                            className={`flex items-center space-x-1 ${isDeafened ? "text-destructive" : "text-[var(--retro-teal)]"}`}
                          >
                      ...
                      <p className="text-muted-foreground retro-text">Kliknij, aby rozpocząć połączenie głosowe.</p>
                    )}
                  </div>
                    ...
                    {isVoiceConnected && (
                      <div className="flex justify-center space-x-3">
                        ...
                        >
                          ...
                          <span className="ml-2 hidden sm:inline">{isMuted ? "Odcisz" : "Wycisz"}</span>
                        </Button>
                        ...
                        >
                          ...
                          <span className="ml-2 hidden sm:inline">{isDeafened ? "Włącz" : "Wyłącz"}</span>
                        </Button>
                      </div>
                    )}
                  </div>
