      ...
      <Card>
        <CardContent className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Nie znaleziono logów spełniających kryteria wyszukiwania</p>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <ScrollArea className="h-[600px]">
          <Table>
            ...
            <TableBody>
              {logs.map((log) => (
                <TableRow key={log.id}>
                  <TableCell className="font-mono text-sm">{new Date(log.createdAt).toLocaleString("pl-PL")}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="font-mono text-sm">
                      {log.actorId ? (
                        <span title={log.actorId}>{log.actorId.slice(-8)}</span>
                      ) : (
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      {log.targetId && (
                        <div className="font-mono text-xs text-muted-foreground">{log.targetId.slice(-8)}</div>
                      )}
                    </div>
                  </TableCell>
